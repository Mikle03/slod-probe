...
def build_pdf(path: Path) -> None:
    ...
    data = [["Condition", "Train/test", "Purpose"], ["In-domain", "NLP → held-out NLP papers", "Primary linear decoding test"], ["Cross-domain", "N/A", "Second domain unavailable"], ["Length-controlled", "NLP → held-out NLP papers", "Remove span-length variation"]]
    ...
    P("The main normal error was micro→macro (38). Under control, macro↔micro confusion dominated (63/60). Meso was not hardest: normal F1=0.912; micro was hardest (0.856 normal, 0.739 controlled). Cross-domain generalization cannot be inferred without a second domain.")
    ...
    P("Next experiments: obtain a second domain; build a small human-labelled SLoD test set; mask section-title expressions; perform tighter content/length matching; compare layers and embedding models; add random-label/selectivity controls; and evaluate calibration and nonlinear probes. These would distinguish semantic abstraction from document-template artifacts.", "Small")
